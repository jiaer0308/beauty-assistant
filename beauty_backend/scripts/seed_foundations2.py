...
JSON_FILE_PATH = Path(__file__).parent.parent / "app" / "data" / "foundations.json"
...
def map_base_makeup_to_seasons(hex_code: str) -> list:
    ...
    Specifically tuned for Foundation, Concealer, and BB/CC creams.
    ...
    # Normalise to [0, 1] then reshape to (1, 1, 3) for skimage
    ...
    # Undertone: positive = warm (yellow/golden), negative = cool (pink/rosy)
    undertone_diff = b - a

    # --- Deep/Dark: rich warm-brown tones (high a* + b*, dark L*) ---
    if L < 50 and (a + b) > 25:
